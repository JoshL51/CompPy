...
class NACA4Profile(QWidget):
    ...
    def stageCalc(self):
        if self.stageObj == 'R':
            ...
            self.rootCamber = (self.stageVars['Root Chord (Rotor)'] / 2 / np.sin(deltaBetaRoot) - self.stageVars[
                'Root Chord (Rotor)'] / 2 / np.tan(deltaBetaRoot)) / self.stageVars['Root Chord (Rotor)']
            self.rootCamber *= -1
            ...
            self.tipCamber = (self.stageVars['Tip Chord (Rotor)'] / 2 / np.sin(deltaBetaTip) - self.stageVars[
                'Tip Chord (Rotor)'] / 2 / np.tan(deltaBetaTip)) / self.stageVars['Tip Chord (Rotor)']
            self.tipCamber *= -1
            ...
            avgBetaRoot = (rotorRoot.beta2 + rotorRoot.beta1) / 2
            deltaBetaRoot = rotorRoot.beta2 - rotorRoot.beta1
            self.rootCamber = (self.stageVars['Root Chord (Stator)'] / 2 / np.sin(deltaBetaRoot) - self.stageVars[
                'Root Chord (Stator)'] / 2 / np.tan(deltaBetaRoot)) / self.stageVars['Root Chord (Stator)']
            self.rootCamber *= -1
            ...
            avgBetaTip = (rotorTip.beta2 + rotorTip.beta1) / 2
            deltaBetaTip = rotorTip.beta2 - rotorTip.beta1
            self.tipCamber = (self.stageVars['Tip Chord (Stator)'] / 2 / np.sin(deltaBetaTip) - self.stageVars[
                'Tip Chord (Stator)'] / 2 / np.tan(deltaBetaTip)) / self.stageVars['Tip Chord (Stator)']
            self.tipCamber *= -1
    ...
    def _profThickness(self, camber, chord, thickness, cpos):
        return 5 * thickness * chord * ((0.2969 * (np.sqrt(self.x / chord))) + (-0.1260 * (self.x / chord)) + (
                    -0.3516 * np.power(self.x / chord, 2)) + (0.2843 * np.power(self.x / chord, 3)) + (
                                                    -0.1015 * np.power(self.x / chord, 4)))
...
if __name__ == '__main__':
    ...
    rotor = {'Hub Diameter': '30.000', 'X Twist (Rotor)': '50.000', 'Blade Thickness (Rotor)': '16',
             'Rotor Diameter': '60', 'Hub Length': '17', 'Blade Clearance': '0', 'Y Twist (Rotor)': '0.000',
             'Root Chord (Rotor)': '1', 'Num of Blade (Rotor)': '24', 'Tip Chord (Rotor)': '10.88'}
    common = {'Reaction (R)': '0.4', 'Mean Line Radius': '47.455', 'Flow (Phi)': '0.691', 'RPM': '30000',
              'Loading (Psi)': '0.482'}
    stator = {"Duct ID": 60, "Duct Length": 14.3, "Duct Thickness": 2, "Num of Blade (Stator)": 13,
              "Mount Can Length": 14.3, "Mount Can Dia": 30, "Mount Can Loc": 0, "Blade Thickness (Stator)": 16,
              "Root Chord (Stator)": 15, "Tip Chord (Stator)": 9.405, "X Twist (Stator)": 50, "Y Twist (Stator)": 0}
