...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.commonValidators = {"RPM": None, "Loading (Psi)": None, "Flow (Phi)": None, "Reaction (R)": None,
                                 "Mean Line Radius": None}
        self.rotorValidators = {"Y Twist (Rotor)": None, "X Twist (Rotor)": None, "Tip Chord (Rotor)": None,
                                "Rotor Diameter": None, "Root Chord (Rotor)": None, "Blade Thickness (Rotor)": None,
                                "Hub Diameter": None, "Hub Length": None, "Blade Clearance": None,
                                "Num of Blade (Rotor)": None}
        self.statorValidators = {"Duct ID": None, "Duct Length": None, "Duct Thickness": None,
                                 "Num of Blade (Stator)": None, "Mount Can Length": None, "Mount Can Dia": None,
                                 "Mount Can Loc": None, "Blade Thickness (Stator)": None, "Root Chord (Stator)": None,
                                 "Tip Chord (Stator)": None, "X Twist (Stator)": None, "Y Twist (Stator)": None}
    ...
    def AddStage(self):
        ...
        # Add new stage vars
        self.rotorVars.append(
            {"Y Twist (Rotor)": "", "X Twist (Rotor)": "", "Tip Chord (Rotor)": "", "Rotor Diameter": "",
             "Root Chord (Rotor)": "", "Blade Thickness (Rotor)": "", "Hub Diameter": "", "Hub Length": "",
             "Blade Clearance": "", "Num of Blade (Rotor)": ""})
        self.statorVars.append({"Duct ID": "", "Duct Length": "", "Duct Thickness": "", "Num of Blade (Stator)": "",
                                "Mount Can Length": "", "Mount Can Dia": "", "Mount Can Loc": "",
                                "Blade Thickness (Stator)": "", "Root Chord (Stator)": "", "Tip Chord (Stator)": "",
                                "X Twist (Stator)": "", "Y Twist (Stator)": ""})
        self.commonVars.append(
            {"RPM": "", "Loading (Psi)": "", "Flow (Phi)": "", "Reaction (R)": "", "Mean Line Radius": ""})
    ...
    def CheckAllStates(self, obj):
        ...
        if obj == "R":
            to_check = [self.commonValidators, self.rotorValidators]
        else:
            to_check = [self.commonValidators, self.statorValidators]

        # Check all values in validators dict
        # If one was ont acceptable, append it to failure list
        for dict in to_check:
            if all(val == 2 for val in dict.values()):
                break
            else:
                for item in dict:
                    if dict[item] != 2:
    ...
    def PlotProfile(self):
        ...
        if wind.exec_():
            if wind.sel == 1:
                ...
                if not self.failed:
                    prof = BladePlot.NACA4Profile(MainWindow, self.commonVars[self.clicked],
                                                  self.rotorVars[self.clicked], "R")
                ...
                # If there was a failure, show the failures
                    ...
                    ErrorWindow(MainWindow, self.failed).show()
            ...
            # If stator was picked
                ...
                # If there was no failure
                if not self.failed:
                    prof = BladePlot.NACA4Profile(MainWindow, self.commongVars[self.clicked],
                                                  self.statorVars[self.clicked], "S")
                    self.R_FrameLayout.addWidget(prof)
                    prof.plotter()
                    self.R_Frame.setLayout(self.R_FrameLayout)
                    prof.close()
    ...
    def Render(self):
        ...
        if wind.exec_():
            ...
            if wind.sel == 1:
                ...
                # If there was no failure
                if not self.failed:
                    rend = RenderWindow.RenderWindow(MainWindow, self.commonVars[self.clicked],
                                                     self.rotorVars[self.clicked], "R", self.wallCheck.isChecked())
