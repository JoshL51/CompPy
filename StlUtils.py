...
def drawCylinder(dia, height, res=25):
    ...
    # Draw Lower Verts
    vertices = [[botOrigin[0] + dia / 2 * np.cos(np.deg2rad((360 / res) * i)),
                 botOrigin[1] + dia / 2 * np.sin(np.deg2rad((360 / res) * i)), botOrigin[2]] for i in range(1, res + 1)]
    # Draw Upper Verts
    vertices += [[topOrigin[0] + dia / 2 * np.cos(np.deg2rad((360 / res) * i)),
                  topOrigin[1] + dia / 2 * np.sin(np.deg2rad((360 / res) * i)), topOrigin[2]] for i in
                 range(1, res + 1)]
...
def drawDuct(innerDia, thickness, height, res=25):
    ...
    outer = [[botOrigin[0] + ((innerDia / 2) + thickness) * np.cos(np.deg2rad((360 / res) * i)),
              botOrigin[1] + ((innerDia / 2) + thickness) * np.sin(np.deg2rad((360 / res) * i)), botOrigin[2]] for i in
             range(1, res + 1)]
    # Draw Upper Outer Verts
    outer += [[topOrigin[0] + ((innerDia / 2) + thickness) * np.cos(np.deg2rad((360 / res) * i)),
               topOrigin[1] + ((innerDia / 2) + thickness) * np.sin(np.deg2rad((360 / res) * i)), topOrigin[2]] for i in
              range(1, res + 1)]

    # Draw Lower Outer Verts
    inner = [[botOrigin[0] + (innerDia / 2) * np.cos(np.deg2rad((360 / res) * i)),
              botOrigin[1] + (innerDia / 2) * np.sin(np.deg2rad((360 / res) * i)), botOrigin[2]] for i in
             range(1, res + 1)]
    # Draw Upper Outer Verts
    inner += [[topOrigin[0] + (innerDia / 2) * np.cos(np.deg2rad((360 / res) * i)),
               topOrigin[1] + (innerDia / 2) * np.sin(np.deg2rad((360 / res) * i)), topOrigin[2]] for i in
              range(1, res + 1)]
...
def drawBlade(camberRoot, camberTip, camberPos, thickness, bladeHeight, twistAngle, rootChord, tipChord, cot):
    # Draw Blade Profile
    bladeProf = NACA4Blade(camberRoot=camberRoot, camberTip=camberTip, camberPos=camberPos, thickness=thickness,
                           bladeHeight=bladeHeight, twistAngle=twistAngle, rootChord=rootChord, tipChord=tipChord,
                           cot=cot)
...
if __name__ == '__main__':
    ...
    scale = cylinder.points.flatten('F')
